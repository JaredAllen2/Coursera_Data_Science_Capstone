...
# capstone_ref[.id==4,(pcontw=length(c(pword3,pword2,pword1))),by=word]
# capstone_ref[.id==3,(pcontw=length(c(pword3,pword2,pword1))),by=word]
# capstone_ref[.id==2,(pcontw=length(c(pword3,pword2,pword1))),by=word]
# capstone_ref[.id==1,(pcontw=length(c(pword3,pword2,pword1))),by=word]
```
...
wordpredict <- function(input) {
  ...
  if (nrow(result) >= 3) {
    ...
    awords <- capstone_ref4[(amatch(wordsin[stringlength],capstone_ref4[,pword1],maxDist=5))&
                              ...
                              (amatch(wordsin[stringlength-2],capstone_ref4[,pword3],maxDist=1)),c("word","gramscore")]
  suggestedwords2 <- rbindlist(list(result,awords),use.names=TRUE, idcol=FALSE)
  result <- dplyr::top_n(suggestedwords2,5,gramscore)
  return(result)
  }
