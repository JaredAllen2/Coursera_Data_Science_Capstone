...
A Kneser-Ney smoothed scoring model was used (thanks to an excellent breakdown of the mathematics by Denny Ceccon on Medium.com<sup>1</sup>) to calculate a likelihood score for all potential subsequent words based on preceding trigram, bigram or unigram input. The equation used to calculate the likelihood score was:
...
After calculation of likelihood scores, all words which occured 4 times or less in the corpora were removed to reduce the size of the final lookup table, resulting in a final reference table with 1,155,561 rows.
